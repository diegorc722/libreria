...
if(strlen($_SESSION['alogin'])==0) {
	...
	if(isset($_POST['change'])) {
		...
		$usuario = $_SESSION['alogin'];
		$sql = "SELECT id FROM administrador WHERE usuario=:usuario and clave=:clave";
		...
		if($query -> rowCount() > 0) {
			$con="UPDATE administrador SET clave=:nuevaclave where usuario=:usuario";
			$chngpwd1 = $dbh->prepare($con);
			$chngpwd1-> bindParam(':usuario', $usuario, PDO::PARAM_STR);
			$chngpwd1-> bindParam(':nuevaclave', $nuevaclave, PDO::PARAM_STR);
			$chngpwd1->execute();
			$msg="Tu Clave ha sido cambiada con exito!!";
		} else {
			$error="Tu Clave actual es incorrecta!!";  
		}
	}
?>
<!DOCTYPE html>
	...
	<script type="text/javascript">
		function valid() {
			if(document.cambio.nuevaclave.value!= document.cambio.confirmarclave.value) {
				alert("La nueva clave y la confirmacion de la clave no coinciden!!");
				...
				<div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3" >
					<div class="panel panel-info">
						...
						<div class="panel-body">
							<form role="form" method="post" onSubmit="return valid();" name="cambio">
